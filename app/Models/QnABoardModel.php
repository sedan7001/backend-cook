...
class QnABoardModel extends Model
{
    //public $db;
    public $container;
    private $b_type;
    ...
    public function getCount($type, $str) {
        ...
        //$sql .= " order by group_num desc, ord asc";
    ...
    public function getList($type, $str, $page, $offset=10) {
        ...
        $sql .= (!empty($type) && !empty($str)) ? " where " .$type. " like :str" : "" ;
        $sql .= " order by group_num desc, ord asc limit :start_limit, :offset";
    ...
    public function getDetail($num) {
        ...
        try{
            ...
            $data['files'] = $stmt_file->fetchAll(PDO::FETCH_OBJ);
    ...
    public function register(array $data) {
        ...
        $sql_update = "UPDATE ".$this->b_type." SET group_num =:group_num, ord=:ord, depth=:depth WHERE num = :num";
        ...
        try{
            ...
            $data['group_num'] = $data['num'];
            $data['ord'] = 0;
            $data['depth'] = 0;

            //답변글을 위해 group_num update
            $stmt_update = $db->prepare($sql_update);
            $stmt_update->bindParam(":num", $data['num'], PDO::PARAM_INT);
            $stmt_update->bindParam(":group_num", $data['group_num'], PDO::PARAM_INT);
            $stmt_update->bindParam(":ord", $data['ord'], PDO::PARAM_INT);
            $stmt_update->bindParam(":depth", $data['depth'], PDO::PARAM_INT);
            $stmt_update->execute();
    ...
    public function registerAnswer(array $data) {


        $db = new \Lib\db();
        $db = $db->connect();

        $sql = "INSERT INTO ".$this->b_type." (id, name, nick, subject, content, regist_day, is_html, hit, group_num, ord, depth)";
        $sql .= " VALUES(:id, :name, :nick, :subject, :content, :regist_day, :is_html, 0, :group_num, :ord, :depth)";

        $sql_update = "UPDATE ".$this->b_type." SET ord=ord + 1 WHERE group_num = :group_num AND ord > :ord";
        ...
        try{
            ...
            $stmt_update->bindParam(":ord", $data['ord'], PDO::PARAM_INT);
            $db->beginTransaction();
            //$this->container->logger->addDebug("[query2::".$stmt_update->queryString."]");
            $stmt_update->execute();
            ...
            $stmt->bindParam(':regist_day',date("Y-m-d(H:i)"), PDO::PARAM_STR);
            $stmt->bindParam(':is_html',$data['is_html'], PDO::PARAM_STR);
            $stmt->bindParam(':group_num',$data['group_num'], PDO::PARAM_INT);
            $stmt->bindValue(':ord',$data['ord'], PDO::PARAM_INT);
            $stmt->bindValue(':depth',$data['depth'], PDO::PARAM_INT);
    ...
    //답변글여부체크
//    public function checkByGroupNum($num) {
//        $db = new \Lib\db();
//        $db = $db->connect();
//
//        $sql = "SELECT num FROM ".$this->b_type;
//        $sql .= " WHERE group_num = (SELECT group_num FROM ".$this->b_type;
//        $sql .= " WHERE num = :num) ";
//
//        try{
//            $data=true;
//
//            $stmt = $db->prepare($sql);
//            $stmt->bindParam(':num',$num, PDO::FETCH_OBJ);
//            //$this->container->logger->addDebug("[query::".$stmt->queryString."]");
//            $stmt->execute();
//            if ($stmt->rowCount() > 1) $data = false;
//            $db = null;
//
//            return $data;
//
