...
class FreeController extends Controller
{
    ...
    public function __construct($container)
    {
        ...
        //print_r($container);
        $this->basicBoardModel = new BasicBoardModel($container, 'cook_free');
        $this->basicRippleModel = new BasicRippleModel($container, 'cook_free');
    }


    //목록
    public function getFrees(Request $request, Response $response, $args) {
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            $data['q'] = $qs;
            //var_dump($this->freeModel->getList($type, $str, $page));
            $data['list'] = $this->basicBoardModel->getListPlusRippleCount($type, $str, $page);
            if (count($data) == 0) {
                $resultData = array('result' => '404', 'message' => 'no data', 'data' => '{"error" : {"text" : "리소스가 없습니다."}}');
                throw new SurveyException(json_encode($resultData,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
            }
            $resultData = array('result' => '200', 'message' => 'free list data', 'data' => $data);
            return $response
                ->withStatus(200)
                //->getBody()
    ...
    //자유게시판상세
    public function getFree(Request $request, Response $response, $args) {
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            $data['ripples'] = $this->basicRippleModel->getList($num);
            $resultData = array('result' => '200', 'message' => 'free data', 'data' => $data);
            return $response
                ->withStatus(200)
                //->getBody()
                ->write(json_encode($resultData,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    ...
    //자유게시판 입력
    public function registerFree(Request $request, Response $response, $args) {
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            $free = [];
            $free['subject'] = filter_var($data['subject'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
            $free['name'] = filter_var($user['name'], FILTER_SANITIZE_STRING);
            $free['is_html'] = filter_var($data['is_html'], FILTER_SANITIZE_STRING);
            $free['nick'] = filter_var($user['nick'], FILTER_SANITIZE_STRING);
            $free['id'] = filter_var($user['id'], FILTER_SANITIZE_STRING);
            if ($free['is_html'] == "y") {
                $free['content'] = filter_var($data['content'], FILTER_SANITIZE_SPECIAL_CHARS);
            } else {
                $free['content'] = filter_var($data['content'], FILTER_SANITIZE_FULL_SPECIAL_CHARS); //htmlspecialchars
            }
            ...
            $directory = $this->container->get('uploads'); ///uploads/free
            ...
            foreach ($uploadedFiles['upfile'] as $upfile) { ///private/var/tmp/phpkAnv90
                ...
                if ($upfile->getError() === UPLOAD_ERR_OK) { //https://securr/features.file-upload.errors.php
                    ...
                    $filename = moveUploadedFile($directory, 'free', $upfile);
                    ...
                    , "id" => $user['id']
                    , "is_del"=>"N");
                    array_push($free['files'], $file_infos);
                } else {
            ...
            $data = $this->basicBoardModel->register($free);

            $resultData = array('result' => '201', 'message' => 'Resource created', 'data' => $data);
            return $response
                ->withHeader('Location','/cook/api/frees')
                ->withStatus(201)
                //->getBody()
                ->write(json_encode($resultData,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    ...
    public function modifyFree(Request $request, Response $response, $args) {
        $num = $args['num'];
        $data = $request->getParsedBody();
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            $free = [];
            $free['num'] = filter_var($num,FILTER_SANITIZE_NUMBER_INT);
            $free['subject'] = filter_var($data['subject'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
            $free['name'] = filter_var($user['name'], FILTER_SANITIZE_STRING);
            $free['is_html'] = filter_var($data['is_html'], FILTER_SANITIZE_STRING);
            $free['nick'] = filter_var($user['nick'], FILTER_SANITIZE_STRING);
            ...
            $data = $this->basicBoardModel->modify($free);
    ...
    public function deleteFree(Request $request, Response $response, $args) {
        $num = $args['num'];
        $data = $request->getParsedBody();
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            //게시글이 지워져야 댓글도 지워지게....
            if ($this->basicBoardModel->delete($free)) {
                $data = $this->basicRippleModel->deleteByParent($free);
            }
    ...
    //자유게시판 입력
    public function registerFreeRipple(Request $request, Response $response, $args) {
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            $resultData = array('result' => '201', 'message' => 'Resource created', 'data' => $data);
            return $response
                ->withHeader('Location', '/cook/api/frees/'.$parent)
                ->withStatus(201)
                //->getBody()
                ->write(json_encode($resultData,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    ...
    public function deleteFreeRipple(Request $request, Response $response, $args) {
        $parent = $args['num'];
        $num = $args['r_num'];
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            $free = [];
            $free['num'] = filter_var($num,FILTER_SANITIZE_NUMBER_INT);
            $free['parent'] = filter_var($parent,FILTER_SANITIZE_NUMBER_INT);
            $free['id'] = filter_var($user['id'], FILTER_SANITIZE_STRING);

            $data = $this->basicRippleModel->delete($free);
