...
class DownloadController extends Controller
{
    ...
    public function __construct($container)
    {
        ...
        //print_r($container);
        $this->basicBoardModel = new BasicBoardModel($container, 'cook_download');
    }


    //목록
    public function getDownloads(Request $request, Response $response, $args) {
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            $resultData = array('result' => '200', 'message' => 'download list data', 'data' => $data);
            return $response
                ->withStatus(200)
                //->getBody()
    ...
    public function getDownload(Request $request, Response $response, $args) {
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            $resultData = array('result' => '200', 'message' => 'download data', 'data' => $data);
            return $response
                ->withStatus(200)
                //->getBody()
                ->write(json_encode($resultData,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    ...
    //자료실 입력
    public function registerDownload(Request $request, Response $response, $args) {
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            $download = [];
            $download['subject'] = filter_var($data['subject'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
            $download['name'] = filter_var($user['name'], FILTER_SANITIZE_STRING);
            $download['is_html'] = filter_var($data['is_html'], FILTER_SANITIZE_STRING);
            $download['nick'] = filter_var($user['nick'], FILTER_SANITIZE_STRING);
            $download['id'] = filter_var($user['id'], FILTER_SANITIZE_STRING);
            if ($download['is_html'] == "y") {
                $download['content'] = filter_var($data['content'], FILTER_SANITIZE_SPECIAL_CHARS);
            } else {
                $download['content'] = filter_var($data['content'], FILTER_SANITIZE_FULL_SPECIAL_CHARS); //htmlspecialchars
            }
            ...
            $directory = $this->container->get('uploads'); ///uploads/download
            $upload_errors = $this->container->get('upload_errors');
            $uploadedFiles = $request->getUploadedFiles();

            if (count($uploadedFiles['upfile']) == 0) {
                $resultData = array('result' => '400', 'message' => '', 'data' => '{"error" : {"text" : "업로드된 파일이 없습니다."}}');
                throw new SurveyException(json_encode($resultData, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
            }
            ...
            foreach ($uploadedFiles['upfile'] as $upfile) { ///private/var/tmp/phpkAnv90
                ...
                if ($upfile->getError() === UPLOAD_ERR_OK) { //https://securr/features.file-upload.errors.php
                    ...
                    $filename = moveUploadedFile($directory, 'download', $upfile);
                    ...
                    , "mime_type" => $mime_type
                    , "file_size" => $file_size
                    , "b_type"=> "cook_download"
                    , "id" => $user['id']
                    , "is_del"=>"N");
                    array_push($download['files'], $file_infos);
                } else {
            ...
            $data = $this->basicBoardModel->register($download);

            $resultData = array('result' => '201', 'message' => 'Resource created', 'data' => $data);
            return $response
                ->withHeader('Location','/cook/api/downloads')
                ->withStatus(201)
                //->getBody()
                ->write(json_encode($resultData,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    ...
    public function modifyDownload(Request $request, Response $response, $args) {
        $num = $args['num'];
        $data = $request->getParsedBody();
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            $download = [];
            $download['num'] = filter_var($num,FILTER_SANITIZE_NUMBER_INT);
            $download['subject'] = filter_var($data['subject'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
            $download['name'] = filter_var($user['name'], FILTER_SANITIZE_STRING);
            $download['is_html'] = filter_var($data['is_html'], FILTER_SANITIZE_STRING);
            $download['nick'] = filter_var($user['nick'], FILTER_SANITIZE_STRING);
            ...
            $data = $this->basicBoardModel->modify($download);
    ...
    public function deleteDownload(Request $request, Response $response, $args) {
        $num = $args['num'];
        $data = $request->getParsedBody();
        ...
        try {       //throw를 던지는 경우는 try catch를 해야함.
            ...
            $data = $this->basicBoardModel->delete($download);
